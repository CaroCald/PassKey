...
extension API {
    ...
    static let passkeyRegisterStart = "/register/start"
    static let passkeyRegisterFinish = "/register/finish"
    static let passkeyLoginStart = "/login/start"
    static let passkeyLoginFinish = "/login/finish"
}

extension API {
    func passkeyRegistrationChallenge(username: String) async throws -> CredentialCreationOptions {
        let request = APIRequest(api: self, method: .post(["username": username]), path: API.passkeyRegisterStart)
        
        return try await request.fetch()
    }
    
    func passkeyAuthenticationChallenge(username: String) async throws -> CredentialAssertionOptions {
        let request = APIRequest(api: self, method: .post(["username": username]), path: API.passkeyLoginStart)
    ...
    private struct RegisterParams: Encodable {
        let username: String
        let data: RegistrationCredential
        
    }
    
    private struct LoginParams: Encodable {
        let username: String
        let data: AssertionCredential
        
    }

    func passkeyRegister(credential: RegistrationCredential) async throws -> AuthResponse {
        let params = RegisterParams(username: "test",data: credential)
        let request = APIRequest(api: self, method: .post(params), path: API.passkeyRegisterFinish)
    ...
    func passkeyRegister(credential: RegistrationCredential, className: String? = nil) async throws -> AuthResponse {
        ...
        return try await passkeyRegister(credential: credential)
    }
    
    func passkeyAuthenticate(credential: AssertionCredential) async throws -> AuthResponse {
        let params = LoginParams(username: "test", data: credential)
        let request = APIRequest(api: self, method: .post(params), path: API.passkeyLoginFinish)
